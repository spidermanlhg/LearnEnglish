    ...
    try:
        ...
        if by_sentence:
            ...
            result = model.transcribe(
                ...
                beam_size=5,  # 增加beam search宽度提高准确性
                ...
                if (segment["end"] - segment["start"] > max_segment_length and "words" in segment) or "words" in segment:
                    ...
                    for i, word in enumerate(words):
                        ...
                        if split_here:
                            # 创建新的分段
                            # 收集并清理文本
                            segment_text = " ".join(w["word"] for w in words[start_idx:i+1])
                    ...
                    if start_idx < len(words):
                        # 收集并清理文本
                        segment_text = " ".join(w["word"] for w in words[start_idx:])
                        cleaned_text = clean_text(segment_text)
            ...
            for i, segment in enumerate(segments):
                ...
                if 'text' in segment and segment['text']:
                    ...
                    name_list.append({"sn":i+1, "name":f"{i+1:03d}.{audiotype}", "text":cleaned_text})
                else:
